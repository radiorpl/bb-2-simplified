...
//                   	 	 Initial   	1    	2    	3    	4   	 5
const int delay_0[] = 		{   0,    	72, 	107, 	143, 	214, 	286 	};
const int delay_1[] = 		{   0,  	54,  	72, 	107, 	143, 	213 	};
const int delay_2[] = 		{   0,  	72, 	107, 	143, 	213, 	286 	};
const int delay_3[] = 		{   0,  	143, 	214, 	286, 	107, 	143 	};
const int delay_send_1[] =  {   0,  	100, 	250,	500, 	750,	1023	};
const int delay_send_2[] =  {   0,  	100, 	250,	500, 	750,	1023		};
const int delay_fb[] =    	{   0,  	1023, 	1023,	1023, 	1023,	1023	};
...
void setup() {
	...
	mixer7.gain(2, 0.25);
	mixer7.gain(3, 0.25);
	mixer8.gain(0, 0.1);	//clean/fx mix clean
	mixer8.gain(1, 0.9);	//fx mixer
    sgtl5000_1.enable();
...
void setDelay(int delay_setting) {
	if ((last_delay_setting != delay_setting) && (delay_setting >= 0) && (delay_setting <= 5)) {
		...
		float gain_send1 = (float)delay_send_1[delay_setting] / 1023.0; //send 1
	  	mixer3.gain(0, gain_send1);
	  	float gain_send2 = (float)delay_send_1[delay_setting] / 1023.0; //send 1
	  	mixer3.gain(1, gain_send2);
	  	float gain_fb = (float)delay_fb[delay_setting] / 1023.0;	//feedback
	  	mixer4.gain(0, gain_fb);
	  	mixer4.gain(1, gain_fb);
	  	mixer4.gain(2, gain_fb);
	  	mixer4.gain(3, gain_fb);
